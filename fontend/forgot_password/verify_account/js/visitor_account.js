document.getElementById("accountForm").addEventListener("submit", async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/verify_visitor', {  // 修正端点名称
            method: 'POST',
        ...
        if (response.ok) {
            // 跳转并传递用户名
            window.location.href = `../option_page/visitor_forgotpassword.html?username=${encodeURIComponent(username)}`;
        } else {
