document.getElementById("accountForm").addEventListener("submit", async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/verify_manager', {  // 注意端口与server.js一致
            method: 'POST',
        ...
        if (response.ok) {
            // 跳转并传递用户名
            window.location.href = `../option_page/manager_forgotpassword.html?username=${encodeURIComponent(username)}`;
        } else {
